  ...
  for(treenum in 1:n_trees) {
    for(yr in (start_year[treenum] + 1):end_year[treenum]){
      ...
      log_ddbh_predicted[treenum,yr] <- intercept[treenum, yr] + 
        ...
        betaCWD * cwd[treenum, yr -1] + # Effect of climate water deficit
        treat_mean[treatment[plotnum[treenum]]] + # Treatment effect
        treat_cwd[treatment[plotnum[treenum]]] * cwd[treenum, yr -1] + # Treatment:CWD interaction
        treat_size[treatment[plotnum[treenum]]] * (log(size[treenum, yr-1]) - obs_ln_mean)/obs_ln_stdv # Treatment:size interaction
